...
sensor_radius = 30; % Sensor coverage radius for isotropic sensors
comm_radius = 60; % Communication radius for network connectivity
prob_sensor=false;
...
population_size = 100;
max_iterations = 60;
initial_num_sensors = 70; % Start with 50 sensors
max_num_sensors = 1000; % Maximum number of sensors to test

% PSO Options
options = optimoptions('particleswarm', 'SwarmSize', population_size, 'MaxIterations', max_iterations, ...
...
for num_sensors = initial_num_sensors:10:max_num_sensors
    ...
    % PSO Optimization
    [optimized_indices, fval] = particleswarm(@(indices) coverage_objective(round(indices), allowed_positions, env_map, sensor_radius, comm_radius, prob_sensor), ...
    ...
    final_coverage = coverge_map(env_map,   optimized_positions,sensor_radius,num_sensors);
...
function coverage_score=coverge_map(env_map,sensor_positions,sensor_radius,num_sensors) % Isotropic sensor model (uniform coverage)
[rows,cols]=size(env_map);      
coverage_map = zeros(rows, cols);
 [X, Y] = meshgrid(1:cols, 1:rows);
        for i = 1:num_sensors
