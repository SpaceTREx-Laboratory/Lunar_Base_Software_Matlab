classdef Fire<handle
   ...
   properties
    ...
    methods (Access=public)
        function obj = Fire(time_onset,time_1MW, time_level_off, time_q_max, time_q_zero,time_q_growth,Loc,Ti,in,k)
            ...
            for i=1:length(Ll)
                ...
                if d>0
                    ...
                    if temp(Cc(Ll(i)),Rc(Ll(i))) < mean(Location.Temp(),'all')
                        temp(Cc(Ll(i)),Rc(Ll(i)))=mean(Location.Temp(),'all');
                    end
                end
            end
            Location.Temp(temp);
